...
⚠️ ⚠️ ⚠️ **本文巨长，正文近 6w 字 (不含题解)，全面阅读并完全理解需数天乃至十数天时间。需要注意的是，「网络流」是力扣上的低频内容，可选择性阅读。**
...
- 大幅修改了「最大流最小割定理」中的「证明 $k=m$」小节的内容，修正了前一天发现的证明过程中的错误 (结论正确，过程有误)。虽然作者认为修正后的证明是正确的，但如果读者仍觉得有问题，还请私信或邮件联系 (主页中有邮箱地址)。
...
4. **证明翔实，有根有据** 。在「最短路」和「最大流」这两个章节中，对所有不易看出的结论均给出了详细的，较为严格的证明。**包括且不限于对 Dijkstra、Bellman-Ford、SPFA、Floyd-Warshall 算法的正确性证明，对「最大流最小割定理」的证明，对 Edmonds-Karp、Dinic 算法复杂度的证明等。** 部分证明着实耗尽了作者的最后一点智商，导致目前 **智商欠费，大脑停机** 。
...
另外，并非所有图论问题都需要「建图」，因为问题的输入本身就包含了图的信息，只不过通常需要组织成适合算法操作的结构，若该输入可直接被算法操作，也就不必再另外「建图」了。例如在「最小生成树」-「Kruskal」一节中，利用输入 $connections$ 即可完成求解，无需额外建图。 
...
「最短路径」算法中，由于最短路是边权的累计，在无法穷尽松弛的算法中，例如 Dijkstra，我们特别强调了其无法处理负边图的特点，且所有的最短路径算法都不能求解负圈图 (有的可以检测出负圈) 。但「最小生成树」比较边权的大小而无「累计」效应，因此负边和负圈都不影响算法的正确性，也就是在讨论「最小生成树」时，我们 **不考虑图是否有圈以及是否有负边** 。
...
> 证明内容整理自B站南大老师蒋炎岩的授课视频: [[算法竞赛入门] 网络流基础: 理解最大流/最小割定理](https://www.bilibili.com/video/BV1Q7411R7ie?spm_id_from=333.999.0.0)。
