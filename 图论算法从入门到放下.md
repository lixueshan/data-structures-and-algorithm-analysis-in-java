...
这是 yuki 最近一段时间学习图论的总结，发出来跟大家分享一点点🤏心得。写这篇文章的契机是前段时间看 Weiss 那本 「数算Java描述」，看着看着发现到图论那章作者就只提供伪代码了 (之前的章节会提供完整的可运行代码)，而且感觉 Weiss 在那一章里什么都提一点，但匆匆抛出几个结论就跑路了，整章看下来比较难受。找了好多资料，包括力扣上的几本相关的 leetbook，都不太满意。于是乎花了点时间探索整理了一下，写成本文。

这篇文章依旧是 **面向初学者** ，或者学习过一些，但感觉不是特别踏实的朋友。这里的「不踏实」指的可能感觉到同一个问题有很多种图论方法可以解决，只知道常用的那种，但题解区有 $n$ 种方法，有的压根没听过；也可以指的是虽然知道怎么做，但相关算法的正确性证明或者复杂度证明，未曾确切地把握过。

本文力求循序渐进，特别突出了各章节之间的联系与过渡。为了使读者能够在每一章学习中都有及时的正反馈，每介绍一个算法，我都会配套一道力扣上的原题 (力扣没有典型题目则采用其他平台的题目)。证明翔实也是本文的一大特点，不过主要集中在「最短路」和「最大流」这两个章节中。本文的主要内容已呈现在「主要内容一览」中，如果朋友们觉得合适，不妨一看。希望朋友们能感受到 yuki 的一点用心。



**本文标题** 意在表达作者的一种希望，即通过本文，看似沉重的图论算法，也能 **重重拿起，轻轻放下** 。

作者能力水平有限，文章虽已审视几轮，所列代码也全都经过验证，但根据之前几篇文章的经验，这一篇也毫无例外地也会出现一些错误。因此既是发文分享，也是向大家请教的一次机会，**文中若有疏漏之处，请各位不吝赐教** 。
...
![graph_mind.png](https://pic.leetcode-cn.com/1655461415-YbBmuc-graph_mind.png)
...
在后续叙述中，我会假定你已熟悉该表所罗列概念。由于半角 "|" 会影响 markdown 的表格，故表格中涉及到数量之处的 $V$ 和 $E$，应为$|V|$ 和 $|E|$ 。
...
※ 空间复杂度主要取决于建图方式，以「邻接矩阵」建图时为 $O(|V|^2)$，以「邻接表」建图时为 $O(|V|+|E|)$ ，表中不再列出。由于半角 "|" 会影响 markdown 的表格，故表格中以 $V$ 表示 $|V|$ ，$E$ 表示 $|E|$ 。
...
※ 通过 $bfs / dfs$ 来 **逐渐标记整张图 (的所有顶点)**  的做法，也被形象地称之为 [**flood fill (泛洪)**](https://en.wikipedia.org/wiki/Flood_fill) 或 seed fill (播种)。其实就是 **「记忆化搜索」** 。
...
| - Dijkstra 小顶堆版 | $O(ElogV)$ | $O(V+E)$                           | No   | Yes         | No(不可检测) |
| - Dijkstra DAG      | $O(V+E)$   | $O(V+E)$                           | Yes  | No (可检测) | No (可检测)  |
| - Bellman-Ford      | $O(VE)$    | $O(V+E)$                           | Yes  | Yes         | No (可检测)  |
| - SPFA (BFM)        | $O(VE)$    | $O(V+E)$                           | Yes  | Yes         | No (可检测)  |
| **带权 APSP**       |            |                                    |      |             |              |
...
[最小生成树 ( *Minimum Spanning Tree, MST* )](https://en.wikipedia.ahmu.cf/wiki/Minimum_spanning_tree): 通常指的无向图中的一个边集，该边集合使得图中所有顶点互相连通，且边权总和最小。由于此边集构成的子图必然无圈，该子图为一棵树，因此称该边集为「最小生成树」。
...
| $while$ 结束时所有顶点最短路径距离被求出                     | $while$ 结束时 MST 所有边权被求出                            |
...
图论中，除了关心顶点到顶点的「最短距离」，整张图的「最小生成树」，人们还关心顶点间的「流量」。将图想象成一张信息网络，信息在顶点之间流动，每条边的权值限制了同一时间承载流量的上限，我们想知道，从一个顶点，到另一个顶点，一次最多能够发送多少流量，这就是同样著名的「最大流」问题。在本节中我们将从一个定理的证明开始，深入讨论该问题，并给出几种求最大流的算法。
...
![image.png](https://pic.leetcode-cn.com/1655468071-uJSGmY-image.png)
...
![image.png](https://pic.leetcode-cn.com/1655468174-XPenzL-image.png)
...
![image.png](https://pic.leetcode-cn.com/1655468253-oLnkxx-image.png)
...
我们采用 $dfs$ 方式找增广路，并将其看作 Ford-Fulkerson 「算法」。现给出 FF 「算法」解决 [P3376 【模板】网络最大流](https://www.luogu.com.cn/problem/P3376) 一题的代码。
...
现给出 EK 算法解决 [P3376 【模板】网络最大流](https://www.luogu.com.cn/problem/P3376) 一题的代码。
