...
| 深度     | nk节点的深度为根到nk节点的唯一路径的长，根的深度为0，树的深度等于有深度最大的树叶的深度，等于该树的高度。<br />x节点的深度：from root to x<br />树的深度：from root to deepest leaf |
...
| 度       | 节点的分叉数量即其子节点个数。<br />叶子节点度为0，二叉树的节点的度不超过2，N个节点的树的度等于边的总数N-1。 |
...
| 概念                             | 描述                                                         |
| -------------------------------- | ------------------------------------------------------------ |
| 二叉树(binary tree)              | 每个节点最多有只能有两个节点的树。                           |
| 满二叉树(full binary tree)       | 每个节点要么为无儿子的叶子节点，要么拥有两个儿子节点(度为0或2) |
| 完全二叉树(complete binary tree) | 第一种描述：所有位置节点的编号都与完美二叉树对应位置的编号相同的二叉树。<br />第二种描述：去掉最后一层后的树为完美二叉树，且最后一层从左到右有连续的节点。<br />完全二叉树特点：<br />若从上到下从左到右编号所有节点，且根节点编号为1，则第i个节点的左儿子编号为2i，右儿子编号为2i+1 |
| 完美二叉树(perfect binary tree)  | 除最后一层叶子节点外所有层上的节点都有两个儿子节点（度为2）。 |

```
说明：
根据不同定义，有时候满二叉树被定义为完美二叉树。
树的层数，高度，深度，也有不同定义，此处采用如下定义：
根为第一层。树的高度等于树的深度，为根节点到最高层树叶的路径长即树的高度(深度)为h，则树的层数h+1，或者说树的层数为k时，树的高度(深度)为k-1。

N个节点的最大二叉树深度：
当每个节点都只有一个子节点时，深度最大，为N-1
```
...
| 目标                                                         | 关系表达式               |
| ------------------------------------------------------------ | ------------------------ |
| **1. k层(深度/高度为k-1)完美二叉树节点数为N**                |                          |
| 1-1. k                                                       | k = log(N+1)             |
| 1-2. N                                                       | N = 2^k - 1              |
| 1-3. 至k-1层的树的节点数M                                    | 2^(k-1) - 1 或 (N-1) / 2 |
| 1-4. 第k层节点数P                                            | 2^(k-1) 或 (N+1) / 2     |
| 1-5. M和P                                                    | P = M + 1                |
| 1-6. M和N                                                    | N = M + P = 2M + 1       |
| 1-7. P和N                                                    | N = M + P = 2P - 1       |
| **2. k层(深度/高度为k-1)完全二叉树节点数为N**                |                          |
| 2-1. k                                                       | k = ⌊logN⌋ + 1           |
| 2-2. N                                                       | 2^(k-1) <= N <= 2^k - 1  |
| 2-3. 上到下左到右从1开始编号，下标为i的节点的左右子节点的下标 | left = 2i right = 2i + 1 |
| 3. k层任意二叉树                                             |                          |
| 3-1. 第k层最大节点数                                         | 完美二叉树时最大         |
| 3-2. 最大节点总数                                            | 完美二叉树时最大         |
...
3-4证明：
  方法1: 
  N个节点的二叉树，每个节点均有左右两个链，共2N条链。除根节点外，所有节点都有一个来自其父节点的链指向它，所以一共使用了N-1条链(边)，于是剩下的2N-(N-1)=N+1即为null链的数量。
  
  方法2:
  null链个数为度0节点乘2加度1节点乘1
  2*n0+n1，由3-3得
...
<img src="https://tva1.sinaimg.cn/large/008i3skNgy1guv1w4aan9j60ea08iaae02.jpg" alt="image-20210927130033275" style="float:left;zoom:55%;" />
...
| 以下是节点嵌套类                                             |                                                              |
| **private** **static** **class** BinaryNode<E>               | 二叉树节点嵌套类                                             |
...
| 以下为AVL树节点嵌套类                                        |                                                              |
| **private** **static** **class** AvlNode<E>                  | AVL树节点嵌套类                                              |
| **public** E element                                         | 字段，本节点元素                                             |
