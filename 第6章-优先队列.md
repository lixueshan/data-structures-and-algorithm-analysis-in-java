...
对于二叉堆，合并两个堆(假设大小都是n)，只需要对2n长度的数组(将两个堆拷贝到一个新的长度为2n的数组中)执行一次堆化操作，时间复杂度是O(n)。而如果使用左式堆，合并复杂度将降为O(logn)。

定义：任意节点的左儿子的零路径长大于等于右儿子的零路径长的堆。

零路径长(null path length)：节点到一个不具有两个儿子的节点的路径长。
...
不同于属于完全二叉树的二叉堆，左式堆是一棵不平衡的二叉树，堆的左侧倾向于更深，因此考虑在根的右路径(指从根开始一直沿着右儿子直到叶子节点的路径)上执行合并操作。事实上可以证明，右路径包含的节点数至多为⌊log(n+1)⌋。

> 定理：在右路径上有r个节点的左式堆至少有2<sup>r</sup>-1个节点。

证明：以数学归纳法证明。

起始验证：当r=1时，为单节点堆，显然满足。
