...
只交换相邻元素，且大小相等时不交换，故大小相等的数字相对位置不变。
...
该方式的时间复杂度分析参考[此篇文章](https://blog.csdn.net/weixin_45502929/article/details/110914032)。
...
同自顶向下非原地/原地的分析类似，只是程序运行过程从递归变为了迭代。四种归并排序的时空复杂度总结如下（待排序序列是数组形式，不考虑链表形式）。
