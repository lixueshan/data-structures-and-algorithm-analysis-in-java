...
※ 关于称呼：若程序中所有的求并操作都是通过本节方法实现，则可严格地称其为按高度求并。但后续将介绍一种find()的优化版本，该优化版本在查询的同时也完成一部分合并操作，应用此版本的查询方法将导致某些时刻rank[root]与树的实际高度不相等，严谨起见，通常称作按“秩(rank)”求并。详细内容在「路径压缩」一节叙述。
