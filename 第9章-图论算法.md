...
Prim算法与Dijkstra算法非常类似，在步骤上甚至仅有一行松弛操作不同。因此以下以表格对比形式给出该算法的描述和过程。
...
| 以BFS的方式访问当前顶点v的所有邻接顶点w，以如下方式松弛dw<br />**dw = min(dw, dv + d(v, w))** | 以BFS的方式访问当前顶点v的所有邻接顶点w，以如下方式松弛dw<br />**dw = min(dw, d(v, w))** |
|                                                              |                                                              |
