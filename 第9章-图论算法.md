...
![floyd-demo](https://tva1.sinaimg.cn/large/008i3skNgy1gxmvvsoed1g31fu0qoe86.gif)

##### 时空复杂度

时间复杂度：三个循环，O(|V|^3)。

空间复杂度：用于保存任意两点间最短路径长的矩阵空间，O(|V|^2)。
...
```java
public Map<Vertex<E>, Map<Vertex<E>, Integer>> allShortestPathFloyd(){
    // 创建最短路径矩阵(以哈希表套哈希表形式)，并初始化。
    Map<Vertex<E>, Map<Vertex<E>, Integer>> interVerticesDist = new HashMap<>();
    for (Map.Entry<Vertex<E>, Map<Vertex<E>, Integer>> entry : adjListsWeighted.entrySet()) {
        ...
        for (Map.Entry<Vertex<E>, Integer> neighbor : entry.getValue().entrySet()) {
            Vertex<E> neighborKey = neighbor.getKey();
            int neighborValue = neighbor.getValue();
            if(key == neighborKey) { // 顶点到自身的边权初始化为0
                value.put(neighborKey, 0);
            }
            // 不存在的边权初始化为Infinity (在构造器中初始化为0，这里修改为Integer.MAX_VALUE表示无穷大，便于后续松弛操作)
            else if(neighborValue == 0) { 
                value.put(neighborKey, Integer.MAX_VALUE);
            }
            else { // 复制存在的边权
                value.put(neighborKey, neighborValue);
            }

        }
        interVerticesDist.put(key, value);
    }
    // 3个for实现floyd算法
        ...
        for (Vertex<E> i : vertices) {
            for (Vertex<E> j : vertices) {
                ...
                if((long)ik + (long)kj < (long)ij){
                    ...
                    interVerticesDist.put(i, neighbors);
                }
            }
        }
    }
    return interVerticesDist;
}
```
