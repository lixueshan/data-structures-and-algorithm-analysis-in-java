...
算法过程中的s到t的增广操作，即正向边删除和反向边增加的操作，不会导致源点s到任意一点v的最短路径距离减少。即残留图G<sub>f</sub>经过一次BFS增广变为G<sub>f'</sub>后，对任意顶点v，源点到v的最短路径距离不会变小，即有δ<sub>f'</sub>v ≥ δ<sub>f</sub>v。以下利用反证法证明。
...
注意，有的文章声称BFS寻找增广路的操作使得增广路递增，这是不对的。根据a)的证明，得到的是非递减的结果，即不变或递增，并非严格递增。这一结论在b)中足以证明任意边第二次成为关键边时其最短路径长至少增加2，由此得到BFS次数的上界。
...
由此，我们证明了Dinic算法中下一分层图的最短路径长度相比前一分层图严格递增。

实际上从⑥可以进一步看出，递增的长度至少为2。
