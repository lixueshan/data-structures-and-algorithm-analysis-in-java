...
※ 主要取决于建图方式，以「邻接矩阵」建图时为 $O(|V|^2)$，以「邻接表」建图时为 $O(|V|+|E|)$ 。 
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        demo.dists = new int[n][n];
        demo.graph = new ArrayList<>();
        for(int i = 0; i < n; i++) demo.graph.add(new ArrayList<>());
        for(int i = 0; i < m; i++) {
            int u = cin.nextInt() - 1, v = cin.nextInt() - 1, weight = cin.nextInt();
            demo.graph.get(u).add(v);
            demo.dists[u][v] = weight;
        }
        System.out.println(demo.maxFlowEK(s, t));
    }
    public long maxFlowEK(int s, int t) {
        long inc = 0, max = 0;
        while((inc = bfs(s, t)) != 0) { // 不断增广
            int cur = t;
            while(cur != s) { // 发送/发回流
                int p = pre[cur];
                dists[p][cur] -= inc; // 发送inc
                dists[cur][p] += inc; // 发回inc
                cur = p;
            }
            Arrays.fill(visited, false); // 重置 visited
            max += inc; // 将每次增广路带来的流量增加到maxFlow上
        }
        return max;
    }
    // bfs 方式增广，增广过程中记录路径顶点的前驱，返回可发送流量
    private int bfs(int s, int t){
        ...
        while(!q.isEmpty()) {
            ...
            for (int v: graph.get(u)) {
                int weight = dists[u][v];
                if(!visited[v] && weight > 0) { // 未访问过的v，且边权大于0
                    pre[v] = u; // 记录前驱
                    q.add(v); // v入队
                    visited[v] = true;
                    c = Math.min(c, weight); // 更新增广路最小边权值
                }
            }
        }
        return isExist ? c : 0;
...
[Dinic's算法(迪尼茨算法)](https://en.wikipedia.org/wiki/Dinic%27s_algorithm)：EK 算法以 $bfs$ 方式实现 FF 方法中的增广操作，在 Dinic 算法中，采用 $bfs$ 与 $dfs$ 结合的方式增广。首先引进 **高度标号 (层次标号)** 和 **分层图** 的概念。以 $bfs$ 算法从 $s$ 到 $t$ ，标记s为第1层，s的邻接顶点为第2层，以此类推t为最后一层。执行一次BFS，赋予每个顶点高度标号信息，此时的图称作分层图。在此分层图内以DFS方式反复寻找增广路并执行增广操作(找到饱和边，发送和发回流)，累计发送流。完成当前分层图的所有增广操作称作一个阶段。一个阶段结束后，重置高度标号信息，再次执行BFS得到新的分层图并重复DFS的增广操作，直到无法分层时说明s到t已无增广路，算法结束，此时得到的发送流总和即为s到t的最大流。
...
1. 为每一个顶点维护一个可用流(flow)，源点s的可用流设置为Integer.MAX_VALUE。该属性表示顶点的可发送流。
