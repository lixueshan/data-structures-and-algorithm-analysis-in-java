...
<img src="https://tva1.sinaimg.cn/large/008i3skNgy1gyd7dgs29zj307m0di0sw.jpg" alt="image" width="100" style="float:left" /> 
...
<img src="https://tva1.sinaimg.cn/large/008i3skNgy1gvys7ulosej308w08mjrb.jpg" alt="image-20211031204826493" width="140" style="float:left;;" />
...
<img src="https://tva1.sinaimg.cn/large/008i3skNly1gyelpsd4sfj30f60cqdga.jpg" alt="image-20211215141921008" width="200" style="float:left;" />
...
命题P：给定一张有k个顶点的图，任意顶点i到任意顶点j之间存在最短路径p(i, j)，若这条路径的点集是{i, v<sub>1</sub>, v<sub>2</sub>, ..., v<sub>n</sub>, j}，其中下标中1~n (n ≤ k - 2)的数字标号表示在Floyd-Warshall算法外层循环中被处理的先后顺序（除i在起点，j在终点外其他顶点的连接顺序是任意的），本命题宣称在外层循环处理顶点v<sub>n</sub>后，i到j的最短路径长度d(i, j)被确定。
...
当n = 0时，表示外层循环尚未开始执行，得到点集为{i, j}的路径p(i, j)，意思是求得所有仅有一条边(i, j)的最短路径，实际上就是所有的边都是这样的路径。这在程序一开始的初始化边权时即得到，故n = 0满足命题。
...
当n = 2时，表示外层循环处理了u<sub>1</sub>, u<sub>2</sub>，得到点集为{i, u<sub>1</sub>, u<sub>2</sub>, j}的路径p(i, j)，意思是求得所有有三条边(i > u<sub>1</sub> > u<sub>2</sub> > j 或 i > u<sub>2</sub> > u<sub>1</sub> > j )的最短路径。外层循环在处理u<sub>2</sub>时，通过中内两个循环，能够更新利用d(i, u<sub>2</sub>) + d(u<sub>2</sub>, j)来更新d(i, j)，这是因为d(i, u<sub>2</sub>)和d(u<sub>2</sub>, j)在n=0或n=1时就已求出，故n = 2满足命题。

此后，n = 3, 4 5...，当n = k时，总能够由d(i, u<sub>k</sub>) + d(u<sub>k</sub>, j)来更新d(i, j)，而p(i, u<sub>k</sub>)和p(u<sub>k</sub>, j)都是p(i, j)的一部分，它们在此前就已被求出。

因此当对于顶点集为{i, v<sub>1</sub>, v<sub>2</sub>, ..., v<sub>n</sub>, j}的路径，当外循环的处理完v<sub>n</sub>时，得到该最短路径。当算法结束时，得到任意两点间的最短路径。因为此时外层循环已经遍历了所有顶点，而任何两点间的路径的点集都是所有顶点组成的集合的子集。证毕。
...
下面通过一个例子观察Floyd算法的动态规划过程，对其正确性可以有更直观的感受。
...
回顾Floyd算法的过程，外层循环执行完第k次，给出由k条边组成的路径，下一次会利用长度为1, 2, ... , k的路径连接出长度为k+1的路径。这就好像在将两个点连成线，然后再将两条单边线作为零件连成长度为2的线，因为具备所有单边线，所以无论长度为2的线是由那鞋单边线组成的，都可以找到并连起来。然后再利用长度为1，2的线作为零件连成长度为3的线，以此类推直到连出所有可能长度的线。
