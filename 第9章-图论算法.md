...
![floyd_demo_202112231607](/Users/lixueshan/Desktop/floyd_demo_202112231607.gif)
...
![截屏2021-12-30 18.47.35](https://tva1.sinaimg.cn/large/008i3skNgy1gxw1wu8pnjj31co0n8q8f.jpg)
...
a) 首先证明EK算法通过BFS寻找的增广路长度非递减，推出任意一次BFS寻找增广路的时间复杂度为O(|E|)。

算法过程中的s到t的增广操作，即正向边删除和反向边增加的操作，不会导致源点s到任意一点v的最短路径距离减少。即残留图G<sub>f</sub>经过一次增广后变为G<sub>f'</sub>后，对任意顶点v，源点到v的最短路径距离不会变小，即有δ<sub>f'</sub>v ≥ δ<sub>f</sub>v。以下利用反证法证明。

(1) 假设某一次s到t的增广后，使得某些顶点到源点的最短路径距离相比增广前变小了，且这其中距离源点s距离最近者为v。则有

δ<sub>f'</sub>v < δ<sub>f</sub>v        ①　

(2) 令u为G<sub>f'</sub>中v的前一个顶点，则有

δ<sub>f'</sub>u + 1= δ<sub>f'</sub>v        ②

如(1)所述，v是我们有意选择的在G<sub>f'</sub>中最短距离相比在G<sub>f</sub>中变小的且距离s最近的顶点，u比v更靠近s，但在G<sub>f'</sub>中相比在G<sub>f</sub>中距离s的最短路径未变小，即有

δ<sub>f</sub>u ≤ δ<sub>f'</sub>u        ③

(3) 假设(u, v) ∈ E<sub>f</sub> ，已知s到v的最短路径距离为δ<sub>f</sub>v，则有

δ<sub>f</sub>u + 1 ≥ δ<sub>f</sub>v        ④

结合①、②、③有 δ<sub>f</sub>u ≤ δ<sub>f'</sub>u → δ<sub>f</sub>u + 1 ≤ δ<sub>f'</sub>u + 1 = δ<sub>f'</sub>v < δ<sub>f</sub>v，即得到

δ<sub>f</sub>u + 1 <  δ<sub>f</sub>v        ⑤

④是由(3)的假设得到的，与由①、②、③得到的 ⑤矛盾，故在(1)假设成立的前提下，(3)的假设(u, v) ∈ E<sub>f</sub>不成立，即(u, v) ∉ E<sub>f</sub>。

(4) 由上述知 (u, v) ∉ E<sub>f</sub>，但如(2)所述，(u, v) ∈ E<sub>f'</sub>，故在G<sub>f</sub>上的增广经过了(v, u)，且此边饱和，导致在G<sub>f'</sub>中产生了反向边(u, v)。于是可知在G<sub>f</sub>中有

δ<sub>f</sub>v + 1 = δ<sub>f</sub>u        ⑥

与⑤矛盾，于是最初(1)的假设不成立，即不存在这样的顶点v，故EK算法寻找的增广路长度非递减。

每次增广，在增广路上的未饱和边会多出一条反向边，故增广操作导致的边数增长不会使总边数超过原来的2倍，即|E'| < 2|E|。又知道增广路长度是非递减的，增广路上界是总边数 ( < 2|E| )，故任意一次BFS寻找增广路的时间复杂度总是O(|E|)。

b) 其次证明BFS寻找增广路的次数为O(|V||E|)次。

(1) 定义增广路中的饱和边为关键边。令某次增广中的(u, v)为关键边，增广后(u, v) ∉ E<sub>f'</sub>，(v, u) ∈ E<sub>f'</sub>。若之后的某次增广后(u, v)再次出现，则此次增广经过(v, u)。

(2) (u, v)第一次成为关键边的增广中有

δ<sub>f1</sub>v = δ<sub>f1</sub>u + 1        ①

(3) (u, v)第二次成为关键边的前提是(v, u)在之后的某次增广路径中为关键边，在该次增广中有

δ<sub>f2</sub>u = δ<sub>f2</sub>v + 1        ②

 (4) 由a)的结论，s到任意顶点的最短路径非递减，有δ<sub>f2</sub>v ≥ δ<sub>f1</sub>v，结合②和①，有 δ<sub>f2</sub>u = δ<sub>f2</sub>v + 1 ≥ δ<sub>f1</sub>v + 1 = δ<sub>f1</sub>u + 2，即

δ<sub>f2</sub>u  ≥ δ<sub>f1</sub>u + 2        ③

也就是说，(u, v)第二次成为关键边时的δu至少比前一次成为关键边时大2。δu最多不超过|V| - 1，故(u, v)可以成为关键边的次数最多为(|V| - 1) / 2。每次增广至少有一条边成为关键边，根据a)中的说明，EK算法过程中边数< 2|E|，故增广次数不会超过(|V| - 1)*|E|，即增广次数复杂度为O(|V||E|)。
...
b) 在分层图中，一次DFS寻找增广路，O(|V|)。由于高度标号加一的判断条件限制，DFS只能沿着高度递增的方向从s到t，因此推进次数最多不超过|V|-1次。

c) 在分层图中，每次DFS增广的结果使得一条高度递增方向上的饱和边消失，此边的反向边增加。如前述，增广路只能沿着高度递增的方向，而分层图不变的情况下反向边是沿着高度递减方向增加的，因此一条高度递增边被删除后不会再出现。高度递增边的数量不大于|E|，于是一个阶段内DFS的次数最多为|E|，即O(|E|)。结合b)可知，一个阶段的总复杂度为单次DFS的复杂度与DFS次数的乘积，即O(|V||E|)。

d) 在EK算法复杂度说明中已证明过，删正向边加反向边的增广操作的结果，使得增广路长度是非递减的。由于分层的影响，路径长又受限于最大层高，因此分层操作最多不会超过|V|次，即O(|V|)。

综上，总时间复杂度为分层次数与每个层次图内的DFS总复杂度的乘积，即O(|E||V|^2)。
